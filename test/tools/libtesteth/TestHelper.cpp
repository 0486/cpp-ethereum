...
void mine(Block &s, BlockChain const &_bc, SealEngineFace *_sealer) {
  ...
  _sealer->onSealGenerated(
      [&](bytes const &sealedHeader) { sealed = sealedHeader; });
...
eth::Network stringToNetId(string const &_netname) {
  ...
  static vector<eth::Network> const networks{
      {eth::Network::FrontierTest, eth::Network::HomesteadTest,
       eth::Network::EIP150Test, eth::Network::EIP158Test,
  ...
  BOOST_ERROR(TestOutputHelper::get().testName() + " network not found: " +
              _netname);
...
vector<fs::path> getFiles(fs::path const &_dirPath,
                          set<string> const _extentionMask,
                          string const &_particularFile) {
...
string executeCmd(string const &_command) {
  ...
  if (exitCode != 0)
    BOOST_ERROR("The command '" + _command + "' exited with " +
                toString(exitCode) + " code.");
...
void checkHexHasEvenLength(string const &_str) {
  if (_str.size() % 2)
    BOOST_ERROR(TestOutputHelper::get().testName() +
                " An odd-length hex string represents a byte sequence: " +
                _str);
}
...
void checkOutput(bytesConstRef _output, json_spirit::mObject const &_o) {
  ...
  if (expectedOutput.find("#") == 0)
    ...
    for (auto const &d : _o.at("out").get_array()) {
      BOOST_CHECK_MESSAGE(_output[j] == toInt(d), "Output byte ["
                                                      << j << "] different!");
      ++j;
    }
...
void checkStorage(map<u256, u256> const &_expectedStore,
                  map<u256, u256> const &_resultStore,
  ...
  for (auto &&expectedStorePair : _expectedStore) {
    ...
    if (resultStoreIt == _resultStore.end())
      ...
      BOOST_CHECK_MESSAGE(expectedStoreValue == resultStoreValue,
                          _expectedAddr << ": store[" << expectedStoreKey
                                        << "] = " << resultStoreValue
  ...
  for (auto &&resultStorePair : _resultStore) {
    if (!_expectedStore.count(resultStorePair.first))
      BOOST_ERROR(_expectedAddr << ": unexpected store key "
                                << resultStorePair.first);
  }
}

void checkCallCreates(eth::Transactions const &_resultCallCreates,
                      eth::Transactions const &_expectedCallCreates) {
  ...
  for (size_t i = 0; i < _resultCallCreates.size(); ++i) {
    ...
    BOOST_CHECK(_resultCallCreates[i].receiveAddress() ==
                _expectedCallCreates[i].receiveAddress());
    BOOST_CHECK(_resultCallCreates[i].gas() == _expectedCallCreates[i].gas());
    BOOST_CHECK(_resultCallCreates[i].value() ==
                _expectedCallCreates[i].value());
  }
}
...
string prepareVersionString() {
  ...
  version += "." + string(DEV_QUOTED(ETH_BUILD_OS)) + "." +
             string(DEV_QUOTED(ETH_BUILD_COMPILER));
  return version;
}
...
RLPStream
createRLPStreamFromTransactionFields(json_spirit::mObject const &_tObj) {
...
dev::eth::BlockHeader constructHeader(
    h256 const &_parentHash, h256 const &_sha3Uncles, Address const &_author,
    h256 const &_stateRoot, h256 const &_transactionsRoot,
    h256 const &_receiptsRoot, dev::eth::LogBloom const &_logBloom,
    u256 const &_difficulty, u256 const &_number, u256 const &_gasLimit,
    u256 const &_gasUsed, u256 const &_timestamp, bytes const &_extraData) {
  RLPStream rlpStream;
  rlpStream.appendList(15);

  rlpStream << _parentHash << _sha3Uncles << _author << _stateRoot
            << _transactionsRoot << _receiptsRoot << _logBloom << _difficulty
            << _number << _gasLimit << _gasUsed << _timestamp << _extraData
            << h256{} << eth::Nonce{};

  return BlockHeader(rlpStream.out(), HeaderData);
}

void updateEthashSeal(dev::eth::BlockHeader &_header, h256 const &_mixHash,
                      h64 const &_nonce) {
